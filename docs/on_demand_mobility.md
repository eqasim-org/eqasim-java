...
- `update-termmination-modes` (optional) defines whether the shares of the added modes are included in the termination criteria (defaults to `true` if termmination criteria are already used in the config, otherwise `false`)
