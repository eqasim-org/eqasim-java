...
Therefore, we propose to simply treat each link in the network as a line that needs to be traversed by all vehicles with a specific duration. This duration, as in the classic BPR function, is directly dependent on the flowDataSet on that link. Our idea is now to track the flowDataSet on each link from the previous iteration(s) and then impose the calculated travel time. The same travel time is used in the routing and decision-making, so that agents will avoid using links with high flowDataSet relative to its capacity, because traversal times are high.

## VDF Module

The main components of this extension are the `VDF(QSim)Module` and the `VDFConfigGroup`. The interval on which flows are evaluated can be configured, but is set to one hour by default. The volume-delay-function used is the classic BPR function that calculates a traversal time based on the free-flowDataSet traversal time and the ratio between detected flowDataSet and link capacity.
...
- Set the flowDataSet capacity of all links to infinity (`1e9`), we effectively disable the queue logic using the `flowCapacityFactor` in the QSim configuration
...
**Attention**: The VDF (default BPR) is defined for a full-size simulation, and so are the capacities in the network. To obtain proper travel times, the observed flows, hence, need to be scaled up if a down-scaled demand is used. This is done through the `capacityFactor` parameter in the config group. It works analogously to QSim's flowDataSet capacity factor. A factor of *0.1* performs the calculations as if the capacities were only *10%* of their nominal values.
...
- Or, we send all link traversals as a batch to the flowDataSet trackers (much more performant), but we lose the ability to analyze link enter/leave events otherwise.
