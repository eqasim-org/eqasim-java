...
//		 Gbl.assertIf( network.getLinks().size()>0 ) ; // otherwise network for mode probably not defined
	...
	public List<? extends PlanElement> calcRoute(final Facility fromFacility, final Facility toFacility, final double departureTime,
		...
		if (toLink != fromLink) {
			...
			{ // To make the estimate correct, we need to add the travel time on the last link
				Node lastStartNode = toLink.getFromNode();
				Node lastEndNode = toLink.getToNode();
				Path lastPath = this.routeAlgo.calcLeastCostPath(lastStartNode, lastEndNode, departureTime + travelTime, person, null);
				travelTime += lastPath.travelTime;
			}
