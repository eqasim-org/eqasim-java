...
final public class QSimComponentsConfig {
    private final static Logger log = LogManager.getLogger(QSimComponentsConfig.class);
    ...
    private void addComponent(Key<?> componentKey) {
        if (keys.contains(componentKey)) {
            log.warn(keyToString(componentKey) + " is already registered. The related engine will be executed multiple times in QSim");
        }
