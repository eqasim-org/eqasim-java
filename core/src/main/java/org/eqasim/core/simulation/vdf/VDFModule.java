...
public class VDFModule extends AbstractEqasimExtension {
	...
	protected void installEqasimExtension() {
		...
		switch (vdfConfig.getHandler()) {
			...
			case Interpolation:
				...
				break;
			case LowMemory:
				FlowConfigGroup flowConfigGroup = FlowConfigGroup.getOrCreate(getConfig());
				if (!flowConfigGroup.isActivated()) {
					LOGGER.warn("LowMemoryHandler requires flow counting to be activated. Activating it now.");
					flowConfigGroup.setActivate(true);
	...
	@Provides
	@Singleton
	public LowMemoryHandler provideLowMemoryHandler(FlowDataSet flowDataSet, VDFScope scope) {
		return new LowMemoryHandler(flowDataSet, scope);
	}
