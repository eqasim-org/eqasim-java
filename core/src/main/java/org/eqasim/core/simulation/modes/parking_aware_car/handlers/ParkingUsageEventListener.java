...
public class ParkingUsageEventListener implements PersonArrivalEventHandler, PersonDepartureEventHandler, MobsimBeforeCleanupListener, MobsimInitializedListener {
    ...
    public void handleEvent(PersonDepartureEvent event) {
        ...
        if(ongoingParking == null) {
            ...
            Verify.verify(ongoingParking.getFirst().linkId().equals(event.getLinkId()), "Person %s is picking the (at %s) car from a different link (%s) where it was previously parked (%s)", event.getPersonId().toString(), String.valueOf(event.getTime()), event.getLinkId().toString(), ongoingParking.getFirst().linkId().toString());
        }
