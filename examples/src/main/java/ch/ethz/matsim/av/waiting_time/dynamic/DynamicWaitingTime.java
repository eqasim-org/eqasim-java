...
public class DynamicWaitingTime implements WaitingTimeCollector, WaitingTime {
	...
	public DynamicWaitingTime(LinkGroupDefinition linkGroupDefinition, LinkWaitingTimeData linkWaitingTimeData,
		...
		this.horizon = (int) Math.floor(1.0 / alpha);

		this.cumulativeValues = new double[horizon][linkGroupDefinition.getMaximumIndex() + 1][numberOfTimeBins];
		this.observationCounts = new int[horizon][linkGroupDefinition.getMaximumIndex() + 1][numberOfTimeBins];
	...
	public void registerWaitingTime(double time, double waitingTime, Id<Link> linkId) {
		...
		if (groupIndex != -1) {
			cumulativeValues[horizonIndex][groupIndex][timeIndex] += waitingTime;
			observationCounts[horizonIndex][groupIndex][timeIndex] += 1;
		}
	}
	...
	public void consolidate() {
		...
		for (int groupIndex = 0; groupIndex < linkGroupDefinition.getMaximumIndex() + 1; groupIndex++) {
			for (int timeIndex = 0; timeIndex < numberOfTimeBins; timeIndex++) {
				cumulativeValues[horizonIndex][groupIndex][timeIndex] = 0.0;
				observationCounts[horizonIndex][groupIndex][timeIndex] = 0;
			}
		}
	}
