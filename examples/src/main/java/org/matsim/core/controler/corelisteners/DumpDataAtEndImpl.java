...
@Singleton
final class DumpDataAtEndImpl implements DumpDataAtEnd, ShutdownListener, IterationEndsListener {
	...
	private void dumpOutputEvents() {
		for (ControlerConfigGroup.EventsFileFormat format : this.controlerConfigGroup.getEventsFileFormats()) {
			try {
				...
				File fromFile = new File(this.controlerIO.getIterationFilename(lastIteration, file));
				try {
	...
	private void dumpOutputTrips() {
		try {
			...
			File fromFile = new File(this.controlerIO.getIterationFilename(lastIteration, Controler.DefaultFiles.tripscsv));
			try {
				Files.copy(fromFile.toPath(), toFile.toPath(), StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.COPY_ATTRIBUTES);
			} catch (IOException e) {
				throw new UncheckedIOException(e);
    ...
    private void dumpOutputLegs() {
        try {
            ...
            File fromFile = new File(this.controlerIO.getIterationFilename(lastIteration, Controler.DefaultFiles.legscsv));
            try {
                Files.copy(fromFile.toPath(), toFile.toPath(), StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.COPY_ATTRIBUTES);
            } catch (IOException e) {
                throw new UncheckedIOException(e);
	...
	private void dumpExperiencedPlans() {
		if (this.config.planCalcScore().isWriteExperiencedPlans() ) {
			try {
				...
				File fromFile = new File(this.controlerIO.getIterationFilename(lastIteration, Controler.DefaultFiles.experiencedPlans));
				try {
					Files.copy(fromFile.toPath(), toFile.toPath(),StandardCopyOption.REPLACE_EXISTING,StandardCopyOption.COPY_ATTRIBUTES);
				} catch (IOException e) {
					throw new UncheckedIOException(e);
