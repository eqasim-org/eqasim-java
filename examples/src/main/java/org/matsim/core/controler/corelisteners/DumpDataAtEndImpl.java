...
@Singleton
final class DumpDataAtEndImpl implements DumpDataAtEnd, IterationEndsListener, ShutdownListener {
	...
	private void dumpOutputEvents() {
		try {
			...
			File fromFile = new File(controlerIO.getIterationFilename(lastIteration, Controler.FILENAME_EVENTS_XML));
			try {
	...
	private void dumpExperiencedPlans() {
		if ( config.planCalcScore().isWriteExperiencedPlans() ) {
			try {
				...
				File fromFile = new File(controlerIO.getIterationFilename(lastIteration, Controler.FILENAME_EXPERIENCED_PLANS));
				try {
					Files.copy(fromFile.toPath(), toFile.toPath(),StandardCopyOption.REPLACE_EXISTING,StandardCopyOption.COPY_ATTRIBUTES);
				} catch (IOException e) {
					throw new UncheckedIOException(e);
